...
class NestMysql2Service {
  ...
  executer(p) {
    return function queryBuilder(db: string) {
      return async function (sqlString: string) {
        const q = `USE ${db};\n` + sqlString;

        console.log(q);
        const [result] = await (p as Pool).query("SELECT 1+1;");
        console.log(result);

        return await (p as Pool).query(q)
      }
    }
  }
